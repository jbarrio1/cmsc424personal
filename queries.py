...
### Output columns: name
queries[1] = """
with MondayFlyers(name,birthdate) as (select name,birthdate, flewon.customerid, flightdate from customers, flewon where EXTRACT(DOW FROM flightdate) = 1 and customers.customerid = flewon.customerid),
uMondayFlyers(name,bday) as (select distinct name, birthdate from MondayFlyers where name not like '_a%' and name not like '_e%' and name not like '_i%' and name not like '_o%' and name not like '_u%'), 
oldest(date) as (select min(bday) from uMondayFlyers)
select name from uMondayFlyers,oldest where bday != oldest.date;
"""
...
ranked as (select flightid, max(count) -  min(count) as var from flightperday group by flightid), top10 as (
select count(r2.flightid) + 1 as rank, r1.flightid, r1.var from ranked as r1 left join ranked as r2 on r2.var > r1.var group by r1.flightid, r1.var order by rank,r1.flightid) 
select * from top10 where rank <=10;
"""
